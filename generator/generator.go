...
// Editions range mirrors protoc-gen-go. The equivalent constants live in
// google.golang.org/protobuf/internal/editionssupport, which is not importable.
const (
...
func NewGenerator(plugin *protogen.Plugin, featureNames []string, cfg *Config) (*Generator, error) {
	...
	plugin.SupportedEditionsMinimum = SupportedEditionsMinimum
	plugin.SupportedEditionsMaximum = SupportedEditionsMaximum
...
func (gen *Generator) Generate() {
	for _, file := range gen.plugin.Files {
		...
		if err := gen.validate(file); err != nil {
...
// validate rejects the layouts vtproto cannot generate correct code for.
func (gen *Generator) validate(file *protogen.File) error {
	var walk func(messages []*protogen.Message) error
	walk = func(messages []*protogen.Message) error {
		for _, message := range messages {
			if isOpaque(message) && gen.cfg.Wrap {
				return fmt.Errorf("%s: the wrap option is incompatible with the opaque API, "+
					"whose struct fields are unexported", message.Desc.FullName())
			}
			for _, field := range message.Fields {
				if isLazy(field) {
					return fmt.Errorf("%s: lazy fields are not supported", field.Desc.FullName())
				}
			}
	...
	return walk(file.Messages)
}

// buildConstraint returns the //go:build constraint for a generated file, and
// the equivalent legacy "// +build" line, which is empty when the constraint
// cannot be expressed in that syntax.
func buildConstraint(userTag string, level gofeaturespb.GoFeatures_APILevel) (constraint, legacy string) {
	if level != gofeaturespb.GoFeatures_API_HYBRID {
		return userTag, userTag
	}
	if userTag == "" {
		return "!protoopaque", "!protoopaque"
	}
	// "(a || b) && !c" has no legacy equivalent.
	return "(" + userTag + ") && !protoopaque", ""
}
...
func (gen *Generator) generateFile(gf *protogen.GeneratedFile, file *protogen.File) {
	...
	switch constraint, legacy := buildConstraint(p.Config.BuildTag, file.APILevel); {
	case legacy != "":
		// Support both forms of tags for maximum compatibility
		p.P("//go:build ", constraint)
		p.P("// +build ", legacy)
	case constraint != "":
		p.P("//go:build ", constraint)
	}
